...
import random
from WCE_Portal.settings import EMAIL_HOST_USER
...
def display_projects(request):
    ...
    opt = 0
    params = {'projects' : project_list,'opt': opt,'info': 'Recent Projects'}
...
def single_project(request,id,slug):
    ...
    Project.objects.filter(id = id).update(visited = True)
    ...
    project_info['linkedin'] = project.linkedin
    # print(project_info)
...
def search(request):
    ...
    for project in projects:
        if query in project.name.lower() or query in project.domain.lower() or query in str(project.year).lower() or query in project.guide.lower():
            ...
            if query in str(project.year).lower():
    ...
    if len(name) == 0 and len(domain) == 0 and len(year) == 0 and len(guide) == 0:
        params['msg'] = 'no'
...
def view_search(request,heading,search):
    ...
    for project in projects:
        ...
        elif heading == 'year' and search.lower() in project.year.lower():
            tmp_dict = {}
            tmp_dict['id'] = project.id
            tmp_dict['name'] = project.name
            tmp_dict['tagline'] = project.tag_line
...
def filters(request):
    ...
    for project in projects:
        ...
        tmp_dict['tagline'] = project.tag_line
        tmp_dict['photo'] = project.photo
        tmp_dict['year'] = project.year
        project_list.append(tmp_dict)

    project_list = sorted(project_list,key= lambda d: str(d['year']),reverse=True)
...
def sign_up_Guide(request):
    if request.method=="POST":
        ...
        if fm.is_valid():
            ...
            # print(email,username,password,first_name,last_name)
            # Remember to change to student
            student = Guide(username = username,name = first_name+" "+last_name, mail = email, password = password)
            student.save()
    else:
        fm=SignUpForm()
...
def stu_verification(request):
    if request.user.is_authenticated:
        ...
        subject = 'Account Verification'
        message = f'Hi ,Your OTP for verification is {student.otp}'
        email_from = 'kunalkadam064@gmail.com'
...
def guide_project_notification(request):
    if request.user.is_authenticated:
        ...
        if request.method=="POST":
            ...
            elif status == 'Accept All Projects':
                print('Accept All');
        ...
        try:
            ...
            project_list = []
              ...
              # disabled accept all btn
            for project in projects:
                if project.visited == False :
                  params['visited_btn'] = project.visited
            # disabled accept all btn   
